...
// Secure Static uploads — check permissions for files to ensure privacy
const { authenticate } = require('./middleware/auth');
const { DepositScreenshot } = require('./models');
app.use('/uploads', (req, res, next) => {
  // Let avatars pass through without checks
  if (req.path.startsWith('/avatars/') || req.path.includes('avatar')) {
    res.setHeader('Cache-Control', 'public, max-age=3600, immutable');
    return next();
  }

  // Authenticate other files
  authenticate(req, res, async () => {
    ...
    // Owner can view their own screenshots
    const filename = path.basename(req.path);
    try {
      const screenshot = await DepositScreenshot.findOne({ where: { filename } });
      if (screenshot && screenshot.userId === req.user.id) {
        res.setHeader('Cache-Control', 'public, max-age=3600, immutable');
        return next();
      }
    } catch (e) {
      logger.error('Error verifying screenshot ownership:', e);
    }
...
app.use('/api/account-activity', authenticate, require('./controllers/wallet.controller').getAccountActivity);
