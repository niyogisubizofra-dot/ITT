...
const Navbar = () => {
  ...
  const NavLinksLeft = () => (
    <>
      ...
      {user && (
        <Link to="/trading" className="block md:inline-block text-brand-text hover:text-brand-primary transition px-3 py-2 font-medium" onClick={() => setIsOpen(false)}>Trading</Link>
      )}
  ...
  const NavLinksRight = () => (
    <>
      {user ? (
        <div className="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-3 mt-2 md:mt-0 pt-2 md:pt-0 border-t md:border-t-0 border-brand-border text-sm">
          <Link to="/deposit-history" className="block text-brand-text hover:text-emerald-400 transition px-2.5 py-1.5 font-medium" onClick={() => setIsOpen(false)}>Deposits</Link>
          <Link to="/withdrawal-history" className="block text-brand-text hover:text-emerald-400 transition px-2.5 py-1.5 font-medium" onClick={() => setIsOpen(false)}>Withdrawals</Link>
          <Link to="/account-activity" className="block text-brand-text hover:text-cyan-400 transition px-2.5 py-1.5 font-medium" onClick={() => setIsOpen(false)}>Activity</Link>
          <Link to="/dashboard" className="block text-brand-text hover:text-brand-primary transition px-2.5 py-1.5 font-medium" onClick={() => setIsOpen(false)}>Dashboard</Link>
          <button onClick={handleLogout} className="text-left md:text-center text-brand-text hover:text-red-400 transition px-2.5 py-1.5 font-medium w-full md:w-auto">Logout</button>
        </div>
      ) : (
