...
import { Search, Filter, Calendar, ArrowLeft, RefreshCw, FileText, CheckCircle, Clock, XCircle, ChevronLeft, ChevronRight, Image as ImageIcon } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-black tracking-tight">Deposit History</h1>
              <p className="text-xs sm:text-sm text-slate-400">View and track all your account deposits</p>
            </div>
          </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-4">
          <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div className="relative md:col-span-2">
              ...
              <input
                type="text"
                placeholder="Search Tx ID, method, or VIP plan..."
                value={search}
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
          {loading ? (
            ...
            <div className="p-12 text-center text-slate-400 space-y-3">
              ...
              <p className="font-semibold text-slate-300">No deposits found</p>
              <p className="text-xs text-slate-500">Try adjusting your search filters or make a new deposit.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-sm text-slate-300">
                <thead className="bg-slate-950/80 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-800">
                  <tr>
                    ...
                    <th className="px-5 py-4">VIP Plan</th>
                  ...
                  {deposits.map((d) => (
                    <tr key={d.id} className="hover:bg-slate-800/30 transition">
                      ...
                      <td className="px-5 py-4 font-bold text-white">
                        ${d.amount.toFixed(2)} <span className="text-xs text-slate-400 font-normal">USDT</span>
                      </td>
                      <td className="px-5 py-4 font-medium text-slate-300">
                        {d.method}
                      </td>
                      <td className="px-5 py-4 text-xs text-slate-400 whitespace-nowrap">
                        {new Date(d.date).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })}
                      </td>
                      <td className="px-5 py-4">
                        {d.screenshotUploaded ? (
