...
import {
  ...
  Send, Image as ImageIcon, Loader2, Megaphone, ShieldCheck, Ban, Eye, Trash2,
...
const AdminDashboard = () => {
  ...
  const knownTabs = ['overview', 'users', 'transactions', 'investments', 'broadcast', 'chat', 'tasks', 'trading'];
  useEffect(() => {
  ...
  // ── Transactions & Deposit Screenshot Verifications ─────────────────────────────────────
  const [depositVerifications, setDepositVerifications] = useState([]);
  const [depositLoading, setDepositLoading] = useState(false);
  const [depositFilter, setDepositFilter] = useState('pending');
  const [depositSearch, setDepositSearch] = useState('');
  const [screenshotModal, setScreenshotModal] = useState(null); // { path, filename, user }
  ...
  const fetchDepositVerifications = async () => {
    setDepositLoading(true);
    try {
      const res = await axios.get('/api/admin/deposits/verifications', {
        params: { status: depositFilter, search: depositSearch }
      });
      const list = Array.isArray(res.data?.deposits)
        ? res.data.deposits
        : Array.isArray(res.data)
        ? res.data
        : [];
      setDepositVerifications(list);
    } catch (err) {
      console.error('Failed to fetch deposit verifications', err);
    } finally {
  ...
  useEffect(() => {
    if (activeTab === 'transactions') {
      fetchPendingTx();
      fetchDepositVerifications();
    }
  }, [activeTab, depositFilter]);

  const handleApproveDeposit = async (txId, notes = 'Approved by admin') => {
    setProcessingDecisionId(txId);
    try {
      await axios.post(`/api/admin/transactions/${txId}/approve`, { adminNotes: notes });
      fetchDepositVerifications();
      fetchPendingTx();
    } catch (err) {
  ...
  const handleRejectDeposit = async () => {
    ...
    setProcessingDecisionId(rejectModalTx.id);
    try {
      await axios.post(`/api/admin/transactions/${rejectModalTx.id}/reject`, { adminNotes: rejectReason || 'Payment verification failed' });
  ...
  // ── Trading Management State ───────────────────────────────────────────────
  const [tradingEnabled, setTradingEnabled] = useState(true);
  const [adminCoins, setAdminCoins] = useState([]);
  const [adminCoinsLoading, setAdminCoinsLoading] = useState(false);
  
  // Coin form states (add & edit)
  const [coinForm, setCoinForm] = useState({ id: null, name: '', symbol: '', currentPrice: '', volatility: 'medium', isActive: true });
  const [coinFormLoading, setCoinFormLoading] = useState(false);
  const [isEditingCoin, setIsEditingCoin] = useState(false);
  
  // Market control form states
  const [marketControlForm, setMarketControlForm] = useState({ coinId: '', priceUpdateType: 'adjust', priceChangePercent: '1.00', isUp: true, marketStatus: 'open', volatility: 'medium', manualPrice: '' });
  const [marketControlLoading, setMarketControlLoading] = useState(false);

  // Fetch admin trading settings and coins
  const fetchAdminTradingData = async () => {
    setAdminCoinsLoading(true);
    try {
      const statusRes = await axios.get('/api/trading/status');
      setTradingEnabled(statusRes.data.enabled);
      
      const coinsRes = await axios.get('/api/admin/trading/coins');
      setAdminCoins(coinsRes.data.coins);
    } catch (err) {
      console.error('Failed to fetch admin trading data', err);
    } finally {
      setAdminCoinsLoading(false);
    }
  };

  useEffect(() => {
    if (activeTab === 'trading') {
      fetchAdminTradingData();
    }
  }, [activeTab]);

  const handleToggleGlobalTrading = async () => {
    try {
      const target = !tradingEnabled;
      await axios.put('/api/admin/trading/settings', { trading_enabled: target });
      setTradingEnabled(target);
      alert(`Trading has been ${target ? 'enabled' : 'disabled'} globally.`);
    } catch (err) {
      alert('Failed to update global trading settings.');
    }
  };

  const handleCoinFormSubmit = async (e) => {
    e.preventDefault();
    setCoinFormLoading(true);
    try {
      if (isEditingCoin) {
        await axios.put(`/api/admin/trading/coins/${coinForm.id}`, coinForm);
        alert('Coin updated successfully.');
      } else {
        await axios.post('/api/admin/trading/coins', coinForm);
        alert('Coin created successfully.');
      }
      setCoinForm({ id: null, name: '', symbol: '', currentPrice: '', volatility: 'medium', isActive: true });
      setIsEditingCoin(false);
      await fetchAdminTradingData();
    } catch (err) {
      alert(err.response?.data?.error || 'Coin operation failed.');
    } finally {
      setCoinFormLoading(false);
    }
  };

  const handleEditCoinClick = (coin) => {
    setCoinForm({
      id: coin.id,
      name: coin.name,
      symbol: coin.symbol,
  ...
  const handleDeleteCoinClick = async (id) => {
    if (!window.confirm('Are you sure you want to permanently delete this coin?')) return;
    try {
      await axios.delete(`/api/admin/trading/coins/${id}`);
  ...
  const handleMarketControlSubmit = async (e) => {
    e.preventDefault();
    if (!marketControlForm.coinId) return alert('Please select a coin first.');
    setMarketControlLoading(true);
    try {
      await axios.post(`/api/admin/trading/coins/${marketControlForm.coinId}/control`, marketControlForm);
      alert('Market control settings applied successfully.');
      await fetchAdminTradingData();
    } catch (err) {
      alert(err.response?.data?.error || 'Market control operation failed.');
    } finally {
      setMarketControlLoading(false);
    }
  };

  const handleSelectMarketControlCoin = (coin) => {
    setMarketControlForm({
      coinId: coin.id,
      priceUpdateType: 'adjust',
      priceChangePercent: '1.00',
      isUp: coin.isUp,
      marketStatus: coin.marketStatus,
      volatility: coin.volatility,
      manualPrice: coin.currentPrice
    });
  };
    ...
    <div className="flex h-screen bg-gradient-to-br from-slate-50 via-slate-50 to-blue-50/40 overflow-hidden font-sans fixed inset-0 z-50">
      ...
      <main className="flex-grow overflow-y-auto w-full">
        ...
        <div className="p-3 sm:p-4 md:p-8 max-w-7xl mx-auto w-full">
          ...
          {/* TRANSACTIONS & DEPOSIT VERIFICATION TAB */}
          {activeTab === 'transactions' && (
            <div className="fade-in space-y-8">
              {/* Deposit Screenshot Verification Panel */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                  <div>
                    <h2 className="text-2xl md:text-3xl font-black text-slate-800 flex items-center gap-3">
                      ...
                      Deposit Verification & Screenshot Review
                    </h2>
                    <p className="text-sm text-slate-500 mt-1">
                      Review payment proofs uploaded by users. Approving will credit user balance & activate VIP plans automatically.
                    </p>
                  </div>

                  <button
                    onClick={fetchDepositVerifications}
                    className="self-start md:self-auto p-2.5 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 transition-colors flex items-center gap-2 text-sm font-semibold"
                  >
                    <RefreshCw className={`w-4 h-4 ${depositLoading ? 'animate-spin' : ''}`} /> Refresh
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="relative md:col-span-2">
                    ...
                    <input
                      type="text"
                      placeholder="Search deposits by reference, user, email, method, or VIP plan..."
                      value={depositSearch}
                      onChange={(e) => setDepositSearch(e.target.value)}
                      onKeyDown={(e) => e.key === 'Enter' && fetchDepositVerifications()}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-emerald-500"
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {['pending', 'approved', 'rejected', 'all'].map((f) => (
                      <button
                        ...
                        className={`flex-1 py-2.5 px-3 rounded-xl text-xs font-bold capitalize transition ${
                          depositFilter === f
                ...
                {depositLoading ? (
                  <div className="py-16 text-center text-slate-400">
                    ...
                    Loading deposit verifications...
                  </div>
                ) : depositVerifications.length === 0 ? (
                  <div className="py-16 text-center text-slate-500 space-y-2">
                    <CheckCircle2 className="w-12 h-12 text-slate-300 mx-auto" />
                    <p className="font-bold text-slate-700">
                      {depositFilter === 'pending' ? 'No pending deposit requests' : 'No deposits found'}
                    </p>
                    <p className="text-xs text-slate-400">
                      {depositFilter === 'pending' ? 'All caught up — no pending deposit verifications in queue.' : `No deposit records match filter "${depositFilter}".`}
                    </p>
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-left text-slate-400 text-xs font-black uppercase tracking-widest border-b border-slate-100">
                          ...
                          <th className="pb-3 pr-4">Ref / Method</th>
                          <th className="pb-3 pr-4">VIP Plan</th>
                          <th className="pb-3 pr-4">Proof Screenshot</th>
                          <th className="pb-3 pr-4">Status</th>
                          <th className="pb-3">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {depositVerifications.map((dep) => (
                          <tr key={dep.id} className="hover:bg-slate-50/70 transition-colors">
                            <td className="py-4 pr-4">
                              <div className="font-bold text-slate-800">{dep.user || 'User #' + dep.userId}</div>
                              <div className="text-xs text-slate-400">{dep.email || '—'}</div>
                            </td>
                            <td className="py-4 pr-4 font-black text-emerald-600">
                              +${dep.amount.toFixed(2)} USDT
                            </td>
                            <td className="py-4 pr-4">
                              <div className="font-mono text-xs font-semibold text-slate-700">{dep.reference || 'DEP-' + dep.id}</div>
                              <div className="text-xs text-slate-400">{dep.method}</div>
                            </td>
                            <td className="py-4 pr-4 font-bold text-violet-600">
                              {dep.vipPlan ? (
                                <span className="inline-block px-2.5 py-0.5 rounded-full text-xs bg-violet-50 border border-violet-200">
                                  {dep.vipPlan}
                                </span>
                              ) : (
                            ...
                            <td className="py-4 pr-4">
                              {dep.screenshotPath ? (
                                <button
                                  onClick={() => {
                                    const filename = dep.screenshotFilename || (dep.screenshotPath ? dep.screenshotPath.split('/').pop() : 'deposit_proof.png');
                                    const src = dep.screenshotPath.startsWith('http')
                                      ? dep.screenshotPath
                                      : `/api/uploads/screenshots/${filename}`;
                                    setScreenshotModal({ url: src, filename, user: dep.user });
                                  }}
                                ...
                                >
                                  <ImageIcon className="w-3.5 h-3.5 text-emerald-400" /> View Proof
                                </button>
                              ) : (
                            ...
                            <td className="py-4 pr-4">
                              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                                ...
                                dep.status === 'rejected' ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'
                              }`}>
                                {dep.status}
                              </span>
                            </td>
                            <td className="py-4">
                              {dep.status === 'pending' ? (
                                <div className="flex items-center gap-2">
                                  <button
                                    disabled={processingDecisionId === dep.id}
                                    onClick={() => handleApproveDeposit(dep.id)}
                                    className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs px-3.5 py-2 rounded-xl transition disabled:opacity-50 flex items-center gap-1 shadow-sm"
                                  >
                                    {processingDecisionId === dep.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
                                    Approve
                                  </button>
                                  <button
                                    disabled={processingDecisionId === dep.id}
                                    onClick={() => setRejectModalTx(dep)}
                                    className="bg-slate-100 hover:bg-rose-50 text-slate-700 hover:text-rose-600 font-bold text-xs px-3.5 py-2 rounded-xl border border-slate-200 transition disabled:opacity-50 flex items-center gap-1"
                                  >
              ...
              {/* General Pending Transactions List */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8">
                <h3 className="text-xl font-bold text-slate-800 mb-6">Pending Withdrawals & General Queue</h3>
                {pendingTx.filter(t => t.type === 'withdrawal').length === 0 ? (
                  <p className="text-sm text-slate-500 italic">No pending withdrawal requests in queue.</p>
                ) : (
                  <div className="space-y-3">
                    {pendingTx.filter(t => t.type === 'withdrawal').map((t) => (
                      <div key={t.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="font-bold text-slate-800">{t.user} <span className="text-slate-400 font-normal">• {t.method}</span></p>
                            <p className="text-xs text-slate-500">{new Date(t.date).toLocaleString()}</p>
                          </div>
                        </div>

                        <div className="flex items-center space-x-4">
                          <span className="font-black text-red-600 text-base">-${t.amount.toFixed(2)}</span>
                          <div className="flex gap-2">
                            <button
                              disabled={processingTxId === t.id}
                              onClick={() => handleTxDecision(t.id, 'approve')}
                              className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs px-3.5 py-2 rounded-xl transition disabled:opacity-50"
                            >
                              Approve
                            </button>
                            <button
                              ...
                              className="bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold text-xs px-3.5 py-2 rounded-xl transition disabled:opacity-50"
                            >
                              Reject
                            </button>
                          </div>
          ...
          {/* High-Resolution Interactive Deposit Screenshot Viewing Modal */}
          <PaymentProofViewerModal
            isOpen={!!screenshotModal}
            onClose={() => setScreenshotModal(null)}
            proofUrl={screenshotModal?.url}
            filename={screenshotModal?.filename}
            userName={screenshotModal?.user}
          />
          ...
          {rejectModalTx && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full text-white space-y-4 shadow-2xl">
                <h3 className="font-bold text-lg text-rose-400">Reject Deposit Request</h3>
                <p className="text-xs text-slate-400">Specify the reason for rejecting user <span className="font-semibold text-white">{rejectModalTx.user}</span>'s deposit of ${rejectModalTx.amount}.</p>

                <div>
                  <label className="block text-xs text-slate-400 mb-1">Rejection Reason / Admin Notes</label>
                  <textarea
                    rows={3}
                    placeholder="e.g. Invalid payment screenshot, unconfirmed transaction ID, or incorrect amount."
                    value={rejectReason}
                ...
                <div className="flex justify-end space-x-3 pt-2">
                  <button
                    onClick={() => { setRejectModalTx(null); setRejectReason(''); }}
                  ...
                  <button
                    ...
                    disabled={processingDecisionId === rejectModalTx.id}
                  ...
                  >
                    {processingDecisionId === rejectModalTx.id ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Confirm Rejection'}
                  </button>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'trading' && (
            <div className="fade-in max-w-6xl mx-auto space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl md:text-3xl font-black text-slate-800">Trading Module Management</h2>
                  <p className="text-slate-500 font-medium mt-1">Configure global trading status, add/edit coins, and control market prices manually.</p>
                </div>
                <div className="p-3 rounded-2xl bg-blue-50 text-blue-600">
              ...
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-slate-800 text-lg">Global Trading Status</h3>
                  <p className="text-slate-400 text-sm mt-0.5">Toggle trading capability for all Client users on the system.</p>
                </div>
                <button
                  onClick={handleToggleGlobalTrading}
                  className={`px-6 py-3 rounded-xl font-bold transition text-xs shadow-md ${
                    tradingEnabled
                      ? 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-emerald-500/10'
                      : 'bg-red-600 text-white hover:bg-red-700 shadow-red-500/10'
                  }`}
                >
                  {tradingEnabled ? 'Trading Enabled (Active)' : 'Trading Disabled (Inactive)'}
                </button>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                
                {/* Coin Form & Selection (Left Panel) */}
                <div className="lg:col-span-1 space-y-8">
                  
                  {/* Create / Edit Coin Card */}
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <h3 className="font-bold text-slate-800 text-base mb-4">
                      {isEditingCoin ? 'Edit Cryptocurrency' : 'Add New Coin'}
                    </h3>
                    <form onSubmit={handleCoinFormSubmit} className="space-y-4">
                      <div>
                        <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Coin Name</label>
                        <input
                          required
                          value={coinForm.name}
                          onChange={(e) => setCoinForm({ ...coinForm, name: e.target.value })}
                          placeholder="e.g. Cardano"
                          className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm font-medium text-slate-700 focus:outline-none"
                        />
                      </div>
                      
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Symbol</label>
                          <input
                            required
                            value={coinForm.symbol}
                            onChange={(e) => setCoinForm({ ...coinForm, symbol: e.target.value })}
                            placeholder="e.g. ADA"
                            className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm font-medium text-slate-700 focus:outline-none uppercase"
                            disabled={isEditingCoin}
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Price (USDT)</label>
                          <input
                            required
                            type="number"
                            step="0.00000001"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Volatility</label>
                          <select
                            value={coinForm.volatility}
                            onChange={(e) => setCoinForm({ ...coinForm, volatility: e.target.value })}
                            className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm font-bold text-slate-650 bg-white focus:outline-none"
                          >
                            <option value="low">Low</option>
                            <option value="medium">Medium</option>
                            <option value="high">High</option>
                        ...
                        <div className="flex items-center space-x-2 pt-4">
                          <input
                            ...
                            onChange={(e) => setCoinForm({ ...coinForm, isActive: e.target.checked })}
                            className="w-4 h-4 rounded text-blue-600 focus:ring-blue-500/20 border-slate-350"
                          />
                          <label htmlFor="isActive" className="text-xs font-bold text-slate-650 cursor-pointer">Active (Visible)</label>
                        </div>
                      </div>
                      ...
                      <div className="pt-2 flex gap-2">
                        ...
                        <button
                          type="submit"
                          disabled={coinFormLoading}
                          className={`font-bold py-2.5 rounded-xl text-xs transition duration-200 ${
                            isEditingCoin ? 'w-1/2 bg-emerald-600 hover:bg-emerald-700 text-white' : 'w-full bg-blue-600 hover:bg-blue-700 text-white'
                          }`}
                        >
                          {coinFormLoading ? 'Saving...' : isEditingCoin ? 'Update Coin' : 'Create Coin'}
                        </button>
                      </div>
                    </form>
                  </div>
                  ...
                  {marketControlForm.coinId && (
                    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm animate-fade-in">
                      ...
                      <form onSubmit={handleMarketControlSubmit} className="space-y-4">
                        <div>
                          <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Update Price Mode</label>
                          <div className="grid grid-cols-2 gap-2 bg-slate-50 p-1 rounded-xl">
                            <button
                              type="button"
                              onClick={() => setMarketControlForm({ ...marketControlForm, priceUpdateType: 'adjust' })}
                              className={`py-1.5 px-3 rounded-lg font-bold text-[10px] transition ${
                                marketControlForm.priceUpdateType === 'adjust' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800'
                              }`}
                            >
                              Percentage Adjust
                            </button>
                            <button
                              type="button"
                              onClick={() => setMarketControlForm({ ...marketControlForm, priceUpdateType: 'manual' })}
                              className={`py-1.5 px-3 rounded-lg font-bold text-[10px] transition ${
                                marketControlForm.priceUpdateType === 'manual' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800'
                              }`}
                            >
                              Direct Price Set
                            </button>
                        ...
                        {marketControlForm.priceUpdateType === 'adjust' ? (
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Change Percent</label>
                              <input
                                required
                                type="number"
                                step="0.01"
                                min="0.01"
                                value={marketControlForm.priceChangePercent}
                                onChange={(e) => setMarketControlForm({ ...marketControlForm, priceChangePercent: e.target.value })}
                                className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm font-medium text-slate-700 focus:outline-none"
                              />
                            </div>
                            <div>
                              <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Direction</label>
                              <select
                                value={marketControlForm.isUp}
                                onChange={(e) => setMarketControlForm({ ...marketControlForm, isUp: e.target.value === 'true' })}
                                className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm font-bold text-slate-650 bg-white focus:outline-none"
                              >
                                <option value="true">Price Increase (Up)</option>
                                <option value="false">Price Decrease (Down)</option>
                              </select>
                            </div>
                          </div>
                        ) : (
                          <div>
                            <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">New Manual Price (USDT)</label>
                            <input
                              required
                              type="number"
                              step="0.00000001"
                              min="0.00000001"
                              value={marketControlForm.manualPrice}
                              onChange={(e) => setMarketControlForm({ ...marketControlForm, manualPrice: e.target.value })}
                              className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm font-medium text-slate-700 focus:outline-none"
                            />
                          </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Market Status</label>
                            <select
                              value={marketControlForm.marketStatus}
                              onChange={(e) => setMarketControlForm({ ...marketControlForm, marketStatus: e.target.value })}
                              className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm font-bold text-slate-650 bg-white focus:outline-none"
                            >
                              <option value="open">Open (Tradable)</option>
                              <option value="closed">Closed (Locked)</option>
                            </select>
                          </div>
                          <div>
                            <label className="block text-[10px] font-black uppercase text-slate-400 mb-1">Volatility Shift</label>
                            <select
                              value={marketControlForm.volatility}
                              onChange={(e) => setMarketControlForm({ ...marketControlForm, volatility: e.target.value })}
                              className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm font-bold text-slate-650 bg-white focus:outline-none"
                            >
                              <option value="low">Low</option>
                              <option value="medium">Medium</option>
                        ...
                        <button
                          type="submit"
                          disabled={marketControlLoading}
                          className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl text-xs transition shadow-md shadow-emerald-500/10"
                        >
                          {marketControlLoading ? 'Applying...' : 'Apply Market Controls'}
                        </button>
                      </form>
                    </div>
                  )}
                ...
                {/* Coin List Panel (Right Panel) */}
                <div className="lg:col-span-2">
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <h3 className="font-bold text-slate-800 text-base mb-6 flex items-center justify-between">
                      <span>Cryptocurrency Roster ({adminCoins.length})</span>
                      <span className="text-[10px] text-slate-400 font-medium italic">Click a coin to configure market controls</span>
                    </h3>
                    
                    {adminCoinsLoading ? (
                      <p className="text-slate-400 text-sm text-center py-10 font-medium">Fetching coin list...</p>
                    ) : adminCoins.length === 0 ? (
                      <p className="text-slate-400 text-sm text-center py-10 font-medium">No coins configured in the database.</p>
                    ) : (
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="border-b border-slate-150 text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                              ...
                              <th className="pb-3 text-right">Trend / Change</th>
                          ...
                          <tbody>
                            {adminCoins.map((c) => {
                              const isSelected = marketControlForm.coinId === c.id;
                              
                              return (
                                <tr 
                                  key={c.id} 
                                  onClick={() => handleSelectMarketControlCoin(c)}
                                ...
                                >
                                  <td className="py-4">
                                    <div className="flex items-center space-x-3">
                                      <div className="bg-slate-800 w-8 h-8 rounded-lg flex items-center justify-center font-bold text-blue-400 text-xs shrink-0">
                                        {c.symbol.substring(0,2)}
                                      </div>
                                      <div>
                                        <p className="font-bold text-slate-800 text-sm">{c.name}</p>
                                        <p className="text-[9px] text-slate-400 uppercase tracking-widest font-black">{c.symbol}</p>
                                      </div>
                                    </div>
                                  </td>
                                  <td className="py-4 text-right">
                                    <p className="font-black text-slate-800 text-sm">${parseFloat(c.currentPrice).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 })}</p>
                                    <p className="text-[8px] text-slate-400 mt-0.5">Vol: {c.volatility.toUpperCase()}</p>
                                  </td>
                                  <td className="py-4 text-right">
                                    <div className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-md ${
                                      c.isUp ? 'text-emerald-600 bg-emerald-50' : 'text-red-650 bg-red-50'
                                    }`}>
                                      {c.isUp ? <ArrowUpRight className="w-3 h-3 mr-0.5" /> : <ArrowDownRight className="w-3 h-3 mr-0.5" />}
                                      <span>{c.isUp ? '+' : '-'}{Math.abs(parseFloat(c.priceChangePercent)).toFixed(2)}%</span>
                                    </div>
                                  </td>
                                  <td className="py-4 text-center">
                                    <span className={`inline-block text-[9px] font-black uppercase px-2 py-0.5 rounded ${
                                      ...
                                      {!c.isActive ? 'Inactive' : c.marketStatus}
                                    </span>
                                  </td>
                                  <td className="py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                    <div className="flex items-center justify-end space-x-2">
                                      <button
                                        onClick={() => handleEditCoinClick(c)}
                                      ...
                                      <button
                                        onClick={() => handleDeleteCoinClick(c.id)}
