...
const Login = () => {
  ...
  const handleSubmit = async (e, customEmail = null, customPass = null) => {
    ...
    try {
      ...
      let errMsg = '';
      if (status === 500) {
        errMsg = `Server Error (500): ${displayMsg || 'Internal Server Error.'}`;
        ...
        errMsg = `Connection Failed: Please verify backend server is running.`;
      } else {
        ...
        {!forgotPassword ? (
          <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="pt-4 border-t border-brand-border space-y-2">
              <p className="text-xs text-brand-muted text-center font-semibold uppercase tracking-wider">Quick Demo Login</p>
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => {
                    setFormData({ email: 'jeremie', password: 'passd123' });
                    handleSubmit(null, 'jeremie', 'passd123');
                  }}
                  className="px-3 py-2.5 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 rounded-xl text-xs font-bold text-blue-400 transition text-center"
                >
                  ⚡ Client: jeremie
                </button>
                <button
                  type="button"
                  onClick={() => {
                    setFormData({ email: 'ishimwe', password: 'passd123' });
                    handleSubmit(null, 'ishimwe', 'passd123');
                  }}
                  className="px-3 py-2.5 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 rounded-xl text-xs font-bold text-emerald-400 transition text-center"
                >
                  🛡️ Admin: ishimwe
                </button>
              </div>
