...
import { CheckCircle2, ArrowLeft, ArrowRight, ShieldCheck, Zap, Wallet, Calendar, Copy, Check, Clock, AlertCircle } from 'lucide-react';
...
const DepositSuccess = () => {
  ...
  useEffect(() => {
    ...
    const fetchTransaction = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.error || 'Unable to retrieve deposit confirmation details.');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-brand-dark flex items-center justify-center text-white">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm font-medium text-slate-400">Loading deposit confirmation...</p>
        </div>
      </div>
    );
  }
  ...
  const txData = transaction || {
    ...
    transactionId: searchParams.get('ref') || 'DEP-CONFIRMED',
    date: new Date().toISOString(),
    method: 'BEP20-USDT',
    vipPlan: searchParams.get('vipPlan') || null,
    ...
    <div className="min-h-screen bg-brand-dark flex items-center justify-center p-4 py-12 relative overflow-hidden text-white">
      ...
      <div className="max-w-lg w-full bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 backdrop-blur-xl shadow-2xl relative z-10 fade-in">
        ...
        <div className="text-center space-y-3 pt-2">
          ...
          <p className="text-sm text-emerald-400 font-semibold max-w-sm mx-auto">
        ...
        <div className="bg-slate-950/80 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-inner">
          <div className="text-center border-b border-slate-800 pb-4">
            <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">Confirmed Amount</span>
          ...
          <div className="space-y-3 text-xs sm:text-sm">
            ...
            <div className="flex justify-between items-center py-1">
              <span className="text-slate-400 font-medium">Deposit Method</span>
              <span className="font-semibold text-slate-300">{txData.method || 'BEP20-USDT'}</span>
            </div>

            {txData.vipPlan && (
              <div className="flex justify-between items-center py-1 bg-violet-500/10 border border-violet-500/20 px-3 py-2 rounded-xl">
                <span className="text-violet-300 font-bold flex items-center gap-1.5">
                  <Zap className="w-4 h-4 text-violet-400 fill-violet-400" /> Activated VIP Plan
                </span>
                <span className="font-black text-violet-300 text-sm">{txData.vipPlan}</span>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
          ...
          >
            View Deposit History <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </div>
    </div>
